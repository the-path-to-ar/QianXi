...
public class UnityARCameraManager : MonoBehaviour {
    ...
    public ARKitWorldTrackingSessionConfiguration sessionConfiguration
    {
        get
        {
            ...
            if (detectionImages != null && isRecogImage)
                config.referenceImagesGroupName = detectionImages.resourceGroupName;
            else
                config.referenceImagesGroupName = "";
    ...
    public void RecogImage()
    {
        isRecogPlane = false;
        isRecogImage = true;
        ResetSession();
    }
    public bool isRecogImage = true;
    public bool isRecogPlane = false;
    public void RecogPlane()
    {
        isRecogPlane = true;
        isRecogImage = false;
        ResetSession();
    ...
    public void ResetSession()
    {
        ...
        var config = sessionConfiguration;
        Debug.Log("图片:" + config.referenceImagesGroupName);

        Debug.Log("检测的水平面:" + config.planeDetection);
        if (config.IsSupported)
        {
